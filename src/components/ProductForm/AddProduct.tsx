...
export default function AddProductForm({
  ...
  const handleSubmit = (values: FormProps) => {
    ...
    setTimeout(() => {
      setLoading(false);
      setProducts([...productsList, payload]);
      onClose();
    }, 1000);
  };
  ...
  return (
    <div
      onClick={onClose}
      className="fixed top-0 left-0 w-screen min-h-screen flex flex-col items-center justify-end md:justify-center bg-black/20 backdrop-blur-md z-50"
    >
      <div
        onClick={(e) => {
          e.stopPropagation();
        }}
        className="p-6 bg-white rounded-lg w-full overflow-auto max-h-[500px] md:max-h-[700px] pb-20 md:max-w-[700px] md:pb-6 "
      >
